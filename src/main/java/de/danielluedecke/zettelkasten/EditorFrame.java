...
public class EditorFrame extends javax.swing.JFrame implements WindowListener, DropTargetListener {
	...
	/**
	 * We need this List-Array to iterate the hyperlinks of an entry. Is needed when
	 * an entry should be edited, and the JList with all the links should be filled
	 */
	 ...
	 *
	 * @return {@code true} if an existing entry is currently edited, {@code false}
	 *         if a new entry is currently edited
	 */
	...
	/**
	 * This variable stores the entry number, if we have an entry which should be
	 * edited
	 */
	...
	/**
	 * Indicated whether a list's content is changed, e.g. filtered. if so, we have
	 ...
	 * quick-input-settings is activated (see CSettings), we do not show the whole
	 * keyword-list at once, but filter the list according to the relevance of the
	 * keywords, divided into four steps.
	 */
	 ...
	 * for the first step, where the keywords of the second step base on the
	 * selected keywords of the first step.
	 */
	...
	private final org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application
			...
			.getResourceMap(EditorFrame.class);
	/**
	 * get the strings for file descriptions from the resource map
	 */
	 ...
	 * entries or editing existing entries. Therefore, we pass the data-class as
	 * parameter, so we can either retrieve information to fill automatically the
	 * textfields (when editing an existing entry) or save the information and add a
	 * new entry to the data-class.
	 *
	 * @param zkn
	 * @param d
	 * @param td
	...
	public EditorFrame(ZettelkastenView zkn, Daten d, TasksData td, AcceleratorKeys ak, Settings s, AutoKorrektur ac,
		...
		// the content of "content" is retrieved from text-selection from the main
		// window.
		if (!editmode) {
			if (content != null) {
	...
	private void initBorders(Settings settingsObj) {
		...
		if (settingsObj.isMacStyle()) {
			...
			jListKeywords.setBorder(ZknMacWidgetFactory.getTitledBorder(
					resourceMap.getString("jListKeywords.border.title"), ColorUtil.colorJTreeText, settingsObj));
			jTextAreaRemarks.setBorder(ZknMacWidgetFactory.getTitledBorder(
					resourceMap.getString("jTextAreaRemarks.border.title"), ColorUtil.colorJTreeText, settingsObj));
			jListLinks.setBorder(ZknMacWidgetFactory.getTitledBorder(resourceMap.getString("jListLinks.border.title"),
					ColorUtil.colorJTreeText, settingsObj));
	...
	private void initDragDropTransferHandler() {
				...
				if (str != null) {
					...
					for (String kw : kws) {
						...
						if (!kw.isEmpty() && !isDoubleKeywords(kw)) {
							...
							jListKeywords.scrollRectToVisible(
									jListKeywords.getCellBounds(keywordListModel.size() - 1, keywordListModel.size()));
		 ...
		 * 
		 * @Override protected String exportString(JComponent c) { String selectedText =
		 * jTextAreaAuthor.getSelectedText(); // if no text selected, quit if
		 * (null==selectedText) return null; // return results return
		 * selectedText.toString().trim(); }
		 * 
		 * @Override protected boolean importString(JComponent c, String str) { // check
		 * for valid drop-string if (str!=null) { // each received string consists of
		 * two lines. the first one with information // about the drag-source and the
		 * drag-operation, the second one with the data // by this we can see whether we
		 * have received entries (i.e. a valid drop) String[] aus = str.split("\n"); //
		 * iterate all dropped authors for (String au : aus) { // check whether author
		 * already exisrs in textfield if (!au.isEmpty() && !checkForDoubleAuthors(au))
		 * { // if not, append author string // therefore, add a new line, but only if
		 * the textfield is not empty // (i.e. we already have an author) if
		 * (!jTextAreaAuthor.getText().isEmpty())
		 * jTextAreaAuthor.append(System.lineSeparator()); jTextAreaAuthor.append(au);
		 * // jTextAreaAuthor.insert(au, jTextAreaAuthor.getCaretPosition()); // set the
		 * modified state setModified(true); } } return true; } return false; }
		 * 
		 * @Override protected void cleanup(JComponent c, boolean remove) { if (remove)
	...
	private void initListeners() {
		 ...
		 * 
		 * @Override public void focusLost(java.awt.event.FocusEvent evt) { // retrieve
		 * opposite component Component oppComp = evt.getOppositeComponent(); // check
		 * for valid valie if (oppComp!=null) { // retrieve component's name. in case we
		 * have the root (when popup is triggered) // the name is null String oppName =
		 * oppComp.getName(); // check whether the focus went to the keyword-textfield
		 * if (oppName!=null && !oppName.equals("jTextFieldFilterKeywordlist")) {
		 * removeHighlights(); } } } });
		 * jTextFieldFilterKeywordlist.addFocusListener(new
		...
		jListQuickInputKeywords.addMouseListener(new java.awt.event.MouseAdapter() {
			...
			public void mousePressed(java.awt.event.MouseEvent evt) {
				if (evt.isPopupTrigger() && !jPopupMenuQuickKeywords.isVisible()) {
					setSegmentPossible((jTextAreaEntry.getSelectedText() != null)
							&& (jListQuickInputKeywords.getSelectedValue() != null));
		...
		jListKeywords.addMouseListener(new java.awt.event.MouseAdapter() {
			...
			public void mousePressed(java.awt.event.MouseEvent evt) {
				if (evt.isPopupTrigger() && !jPopupMenuKeywords.isVisible()) {
					setSegmentPossible(
							(jTextAreaEntry.getSelectedText() != null) && (jListKeywords.getSelectedValue() != null));
					focusowner = Constants.FOCUS_FIELD_ADDKEYWORDS;
					jPopupMenuKeywords.show(jListKeywords, evt.getPoint().x, evt.getPoint().y);
				}
			}
	...
	public final void initToolbarIcons() {
		...
		javax.swing.JButton toolbarButtons[] = new javax.swing.JButton[] { tb_cut, tb_copy, tb_paste, tb_selectall,
				tb_undo, tb_redo, tb_newauthor, tb_manlink, tb_footnote, tb_insertimage, tb_inserttable,
				tb_insertattachment, tb_bold, tb_italic, tb_underline, tb_strike, tb_textcolor, tb_highlight };
		String[] buttonNames = new String[] { "tb_cutText", "tb_copyText", "tb_pasteText", "tb_selectallText",
				"tb_undoText", "tb_redoText", "tb_newAuthorText", "tb_manlinkText", "tb_footnoteText",
				"tb_insertimageText", "tb_inserttableText", "tb_insertattachmentText", "tb_boldText", "tb_italicText",
				"tb_underlineText", "tb_strikeText", "tb_textcolorText", "tb_highlightText" };

		String[] iconNames = new String[] { "cutIcon", "copyIcon", "pasteIcon", "selectAllIcon", "undoIcon", "redoIcon",
				"newAuthorIcon", "insertManlinkIcon", "insertFootnoteIcon", "insertImageIcon", "insertTableIcon",
				"insertAttachmentIcon", "formatBoldIcon", "formatItalicIcon", "formatUnderlineIcon",
		...
		// finally, we have to manuall init the actions for the popup-menu, since the
		// gui-builder always
		// puts the menu-items before the line where the action-map is initialised. we
		// cannot change
		// this because it is in the protected area, and when changing it from outside,
		// it will
	...
	private void makeMacToolbar() {
		...
		if (settings.getShowIcons() || settings.getShowIconText()) {
			...
			mactoolbar.addComponentToLeft(
					MacToolbarButton.makeTexturedToolBarButton(tb_cut, MacToolbarButton.SEGMENT_POSITION_FIRST));
			mactoolbar.addComponentToLeft(
			...
			mactoolbar.addComponentToLeft(
					MacToolbarButton.makeTexturedToolBarButton(tb_undo, MacToolbarButton.SEGMENT_POSITION_FIRST));
			mactoolbar.addComponentToLeft(
			...
			mactoolbar.addComponentToLeft(
					MacToolbarButton.makeTexturedToolBarButton(tb_manlink, MacToolbarButton.SEGMENT_POSITION_FIRST));
			mactoolbar.addComponentToLeft(
					MacToolbarButton.makeTexturedToolBarButton(tb_footnote, MacToolbarButton.SEGMENT_POSITION_MIDDLE));
			mactoolbar.addComponentToLeft(MacToolbarButton.makeTexturedToolBarButton(tb_insertimage,
					MacToolbarButton.SEGMENT_POSITION_MIDDLE));
			mactoolbar.addComponentToLeft(
					MacToolbarButton.makeTexturedToolBarButton(tb_inserttable, MacToolbarButton.SEGMENT_POSITION_LAST));
			mactoolbar.addComponentToLeft(MacWidgetFactory.createSpacer(16, 1));
			mactoolbar.addComponentToLeft(
			...
			if (settings.getShowAllIcons()) {
				mactoolbar.addComponentToLeft(MacToolbarButton.makeTexturedToolBarButton(tb_underline,
						MacToolbarButton.SEGMENT_POSITION_MIDDLE));
				mactoolbar.addComponentToLeft(
						MacToolbarButton.makeTexturedToolBarButton(tb_strike, MacToolbarButton.SEGMENT_POSITION_LAST));
			} else {
				mactoolbar.addComponentToLeft(MacToolbarButton.makeTexturedToolBarButton(tb_underline,
	...
	private void initFields() {
		...
		text = text.replace(Constants.FORMAT_NUMBEREDLIST_OPEN,
				Constants.FORMAT_NUMBEREDLIST_OPEN + System.lineSeparator());
	 ...
	 * property "textfieldFilled", which enables or disables the apply-button, is
	 * only changed when the document of the jTextAreaEntry is changed. This should
	 * prevent applying empty content to a new entry.<br>
	 * <br>
	 ...
	 * depending on whether the main textfield (jTextAreaEntry) has the focus or
	 * not. Formatting action only apply to the maintextfield, so disable then when
	 * other fields have the input focus.
	 *
	 * @param focus indicates whether the <b>main textfield</b> has the focus or
	 *              nor. if this parameter is false, the undo/redo buttons are
	 *              always disabled. if the value is true, the buttons are enabled
	 *              when undo/redo is possible (canUndo() and canRedo()).
	 */
	 ...
	 * part so we can call this method in the documents change listener. The
	 * undo/redo buttons are then also immediately being updated when the user types
	 * text.
	 *
	 * @param focus indicates whether the main textfield has the focus or nor. if
	 *              this parameter is false, the undo/redo buttons are always
	 *              disabled. if the value is true, the buttons are enabled when
	 *              undo/redo is possible (canUndo() and canRedo()).
	 */
	...
	/**
	 * This method checks which textfield has the focus and then selects the whole
	 * text in that component.
	 */
	@Action
	...
	public void addAuthorFromMenu() {
		...
		if ((newAu != null) && (newAu.length() > 0)) {
			...
			for (String a : authors) {
				// check whether string is empty or not (may occur when the user uses two new
				// lines
				...
				if (!a.isEmpty()) {
					...
					if (-1 == data.getAuthorPosition(a)) {
						...
						JOptionPane.showMessageDialog(this, resourceMap.getString("errValueExistsMsg"),
								resourceMap.getString("errValueExistsTitle"), JOptionPane.PLAIN_MESSAGE);
					}
				}
			}
			// copy new added authors into a global array, so we can select the new addes
			// authors
	...
	private void selectNewAddedAuthors() {
		if (selectedAuthors != null) {
			...
			// sort the array, so when making the first entry in the *array* visible, this
			// should also be the
	...
	public void addSelectionToSteno() {
		...
		// into our variable - so we avoid code-duplication, and not using for each
		// textarea
	...
	/**
	 * This is a small method which surrounds the currently selected text with tags
	 * which are supplied as parameters
	 *
	...
	private void removeAlignment() {
		...
		selection = selection.replace(Constants.FORMAT_ALIGNLEFT_OPEN, "").replace(Constants.FORMAT_ALIGNLEFT_CLOSE,
				"");
		selection = selection.replace(Constants.FORMAT_ALIGNJUSTIFY_OPEN, "")
				.replace(Constants.FORMAT_ALIGNJUSTIFY_CLOSE, "");
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have italic-formatting.
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have underline-formatting.
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have strike-through-formatting.
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have a header 1st grade
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we a header 2nd grade
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case the text is aligned centered.
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have left and right border margins.
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have a quotation or citation.
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have a inline quotation marks.
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have a code blocks
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have a inline-code blocks
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have changed the text-color.
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have another text-background-color
	 */
	@Action(enabledProperty = "focus")
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have a list with bullet points
	 */
	@Action(enabledProperty = "focus")
	...
	private void insertList(String listTypeOpenTag, String listTypeCloseTag) {
		...
		if (null == selection) {
			// if we don't have any selection, just insert tags
			jTextAreaEntry.replaceSelection(listTypeOpenTag + linesep + Constants.FORMAT_LISTITEM_OPEN
					+ Constants.FORMAT_LISTITEM_CLOSE + linesep + listTypeCloseTag);
		} else {
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have text-superscript-formatting.
	 */
	@Action(enabledProperty = "focus")
	...
	public void formatFont() {
		...
		if (fontDlg.selectedFont != null) {
			...
			surroundSelection(Constants.FORMAT_FONT_OPEN + " " + lastSelectefFont.getFontName() + "]",
					Constants.FORMAT_FONT_CLOSE);
		}
	...
	/**
	 * Retrieves the text selection from the maintextfield and sourrounds it with
	 * the related format-tags. In this case we have text-subscript-formatting.
	 */
	@Action(enabledProperty = "focus")
	...
	public void insertTable() {
		// in case the caret is inside an existing table, we assume the user wants to
		// edit
		...
		// table content and fill the insertTableDialog with the content of the
		// available
	...
	/**
	 * This method inserts a footnote-tag, which inserts references to authors. the
	 * footnote-tags can only be inserted if at least one author is selected.
	 */
			...
			if (tr.isDataFlavorSupported(DataFlavor.javaFileListFlavor) || tr.isDataFlavorSupported(urlFlavor)) {
				...
				if (tr.isDataFlavorSupported(urlFlavor)) {
					...
					if (FileOperationsUtil.isHyperlink(url.toString())) {
						...
						jListLinks.scrollRectToVisible(
								jListLinks.getCellBounds(linkListModel.size() - 1, linkListModel.size()));
						// set the modified state
						setModified(true);
						skip = true;
					}
					...
					if (files != null && files.size() > 0) {
						...
						for (Object file1 : files) {
							...
							if (file.isFile()) {
								...
								if (FileOperationsUtil.isSupportedImageFile(file)
										&& DROP_LOCATION_TEXTAREAENTRY == droplocation) {
									imgfiles.add(file);
								} else {
				...
				Constants.zknlogger.log(Level.WARNING,
						"DataFlavor.javaFileListFlavor or Data.Flavor.URL is not supported, rejected");
				dtde.rejectDrop();
			}
	...
	public class TableContentPane extends JPanel {
		...
		public TableContentPane() {
			...
			// create action for enter-key. by doing this instead of using a key-event,
			// hitting
			// the enter-key does not select a new cell, so another symbol than the intended
			// one
			// is inserted
			AbstractAction a_enter = new AbstractAction() {
				@Override
	...
	/**
	 * This method removes double space chars from the selection of the text field
	 * that currently has the input focus, and replaces them by single space chars
	 */
	@Action(enabledProperty = "textSelected")
	...
	/**
	 * This method removes tabulator chars from the selection of the text field that
	...
	/**
	 * This method open a file-chooser dialog and lets the user choose image-files,
	 * which will be inserted in the textfield. the image-file-chooser gives the
	 * user a preview of the image. when an image-file is chosen, the image-file is
	 * copied to an own image-directory ("/img/") which is a subdirectory of the
	 * main-data-file's directory. if the directory doesn't already exist, it will
	 * be created.
	 */
	@Action(enabledProperty = "focus")
	...
	private void insertImages(File[] sources) {
		...
		for (File cf : sources) {
			...
			if (!cf.toString().startsWith(simagedir)) {
				...
				// now delete everything of that filepath from the beginning to the image
				// directory.
				// we add +1 one here, because the image directory string does not have the
				// trailing
		...
		int msgOption = JOptionPane.showOptionDialog(this, resourceMap.getString("msgConfirmImageCopyMsg", simagedir),
				resourceMap.getString("msgConfirmImageCopyTitle"), JOptionPane.YES_NO_CANCEL_OPTION,
		...
		if (ATT_COPY == msgOption || ATT_MOVE == msgOption) {
			...
			if (!imagedir.exists()) {
				...
				try {
					if (!imagedir.mkdir()) {
						...
						JOptionPane.showMessageDialog(this, resourceMap.getString("errMsgCreateImgDirMsg", imagedir),
			...
			for (File f : sources) {
				...
				while (dest.exists()) {
					...
					Object fnobject = JOptionPane.showInputDialog(this, resourceMap.getString("msgFileExists"),
							resourceMap.getString("msgFileExistsTitle"), JOptionPane.PLAIN_MESSAGE, null, null,
							dest.getName());
					// if the user cancelled the dialog, quit method
					if (null == fnobject) {
						return;
				...
				try {
					...
					if (ATT_COPY == msgOption) {
						...
						if (!f.renameTo(dest)) {
							// ... show error msg
							JOptionPane.showMessageDialog(this, resourceMap.getString("errMsgFileMove"),
	 ...
	 * edited entry. Attachments' filepaths will be used and relative path will be
	 * calculated if necessary, and the filepaths will be added to the attachment
	 * list.
	 */
	@Action
	...
	private void insertAttachments(File[] sources) {
		...
		if (mod) {
			...
			if (linkListModel.size() > 0) {
				// scroll jList down, so the new added links become visible
				jListLinks
						.scrollRectToVisible(jListLinks.getCellBounds(linkListModel.size() - 1, linkListModel.size()));
			}
		}
	}
	 ...
	 * removes all entries that don't have the entered text as part of their string.
	 */
	@Action
	...
	public void filterAuthors() {
		...
		// size which elements in fact no longer exist, because some elements have
		// already
	...
	public void closeWindow() {
		...
		if (modified) {
			...
			int option = JOptionPane.showConfirmDialog(this, confirmExitText, confirmExitTitle,
					JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

			if (option == JOptionPane.CANCEL_OPTION
					|| option == JOptionPane.CLOSED_OPTION /* User pressed cancel key */) {
				// User cancelled the warning. Leave the edit window open.
				return;
			} else if (option == JOptionPane.YES_OPTION) {
				// User said Yes, we should save the entry.

				// If no content entered, tell user that entry needs to have content and leave
				// the edit window open.
				if (!textfieldFilled) {
					JOptionPane.showMessageDialog(this, resourceMap.getString("errMsgNoContentMsg"),
							resourceMap.getString("errMsgNoContentTitle"), JOptionPane.PLAIN_MESSAGE);
					return;
				}
				...
				// User said No, we should not save the entry.

				// Make modified false as nothing changed.
				setModified(false);
			}
		}
	...
	/**
	 * This method is called when the user presses the "Apply" button to save the
	 * changes. It is the action of the apply button, which is only clickable when
	 * textfieldFilled (we have text inside the main jTextAreaEntry)
	 */
	@Action(enabledProperty = "textfieldFilled")
	 ...
	 * respectively changes an entry which was edited. This method is called when
	 * the user presses the ok-button to finish the data-entry, closing the dialog
	 * and savign the entry to the data-file.
	 */
	private boolean saveEntry() {
		...
		// now we have to "clean" the lists. since each bullet point starts for optical
		// reasons
		// in a new line, but in HTML-conversion this would mean additional <br>-tags
		// within and
		// unorder list (<ul> and <li>), we remove all [br]-tags between the list tags.
		content = content.replace(Constants.FORMAT_NUMBEREDLIST_OPEN + Constants.FORMAT_NEWLINE,
				Constants.FORMAT_NUMBEREDLIST_OPEN);
		content = content.replace(Constants.FORMAT_NUMBEREDLIST_CLOSE + Constants.FORMAT_NEWLINE,
				Constants.FORMAT_NUMBEREDLIST_CLOSE);
		...
		content = content.replace(Constants.FORMAT_LISTITEM_CLOSE + Constants.FORMAT_NEWLINE,
				Constants.FORMAT_LISTITEM_CLOSE);
		// check whether all chars are legal JDOM-chars. when copied text from
		// clipboard,
		...
		if (isEditMode()) {
			...
			// here we have the user editing an "deleted" entry (empty place in the
			// XML-document),
			// so the variable "entryNumber" indicates where in the *database* the entry
			// should be stored.
			if (isDeleted) {
				...
				if (result == Daten.ADD_LUHMANNENTRY_ERR) {
					JOptionPane.showMessageDialog(this, resourceMap.getString("errMsgInsertEntry"),
							resourceMap.getString("errMsgInsertEntryTitle"), JOptionPane.PLAIN_MESSAGE);
				} // else check whether an error occured when adding a new entry
				else if (result == Daten.ADD_ENTRY_ERR) {
					JOptionPane.showMessageDialog(this, resourceMap.getString("errMsgAddEntry"),
							resourceMap.getString("errMsgAddEntryTitle"), JOptionPane.PLAIN_MESSAGE);
				} else {
					// tell about success
				...
				// In this case, the variable "entryNumber" indicates whether we have a trailing
				// entry or not.
			else {
				...
				// else, we pass the number of the current entry just before the user clicked
				// "insertEntry"
				// to the addEntry-method, indicating that the index-number of the added entry
				// should be
	...
	/**
	 * This action retrieves the keywords of the currently displayed entry from the
	 * main frame and adds them as keywords to the currently edited entry, i.e.
	 * these values are added to the {@link #jListKeywords jListKeywords}.
	 */
	@Action
	 ...
	 * of the edited entry, i.e. these values are addes to the {@link #jListKeywords
	 * jListKeywords}.
	 *
	 * @param kws              the keyword-values that should be added to the
	 *                         {@link #jListKeywords jListKeywords}.
	 * @param focusToMainfield {@code true} if the {@link #jTextAreaEntry
	 *                         jTextAreaEntry} should retrieve the input focus after
	 *                         adding the keywords, {@code false} if the
	 *                         {@link #jTextFieldAddKeyword jTextFieldAddKeyword}
	 *                         should become the focus-owner after adding the
	 *                         keywords.
	 */
	...
	/**
	 * This method checks a given string if it is already in the author textfield,
	 * i.e. it checks whether an author was already assigned to that entry. If the
	 * given string is found within the author-textfield, this method returns true,
	 * false otherwise.
	 *
	 ...
	 * it checks whether a keyword was already assigned to that entry. If the given
	 * string is found within the keywordlist, this method returns true, false
	 * otherwise.
	 *
	...
	/**
	 * This methods looks for selected entries in the keywordlist and removes them
	 * from it
	 */
	...
	/**
	 * This methods looks for selected entries in the linklist and removes them from
	 * it
	 */
	...
	/**
	 * This method retrieves the selected item from the quickinput-author list and
	 * sets it to the textarea with the author information. This method is e.g.
	 * called from the mouseclick- or keypressed-event from the
	...
	public Task<?, ?> quickInputAuthor() {
		...
		return new QuickInputAuthorTask(
				org.jdesktop.application.Application.getInstance(de.danielluedecke.zettelkasten.ZettelkastenApp.class));
	}
	 ...
	 * dependent on that, most toolbar/menu-actions are being enabled or disabled.
	 */
	...
	/**
	 * This variable indicates whether the we have text in the main textfield. This
	 * is necessary to enable/disable the apply-button, which should only be enabled
	 * when we have text in the main textfield. an entry must have content,
	 * otherwise it will be considered as deleted.
	 */
	...
	/**
	 * This variable indicates whether undo/redo is possible. This is the case when
	 * the main text fiel (jTextAreaEntry) has the focus and changes have been made.
	 */
					...
					for (String ft : findterms) {
						...
						while ((pos = doctext.indexOf(ft.toLowerCase(), pos)) >= 0) {
							try {
								...
								hilite.addHighlight(pos, pos + ft.length(),
										new MyHighlightPainter(new Color(255, 255, 102)));
	...
	/**
	 * This method refreshes the keywordlist for the quick-input, and apllies the
	 * keyword-search on the text-selection only. thus, this method is only enabled
	...
	private void initComponents() {
		...
		jToolBarNewEntry.setBorder(javax.swing.BorderFactory.createMatteBorder(0, 0, 1, 0,
				resourceMap.getColor("jToolBarNewEntry.border.matteColor"))); // NOI18N
		...
		jTextAreaEntry.setBorder(
				javax.swing.BorderFactory.createTitledBorder(resourceMap.getString("jTextAreaEntry.border.title"))); // NOI18N
		...
		jTextAreaAuthor.setBorder(
				javax.swing.BorderFactory.createTitledBorder(resourceMap.getString("jTextAreaAuthor.border.title"))); // NOI18N
		...
		jListKeywords.setBorder(
				javax.swing.BorderFactory.createTitledBorder(resourceMap.getString("jListKeywords.border.title"))); // NOI18N
		...
		jPanel4Layout
				.setVerticalGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel4Layout.createSequentialGroup()
								...
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jTextFieldAddKeyword, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
								.addContainerGap()));
		...
		jTextAreaRemarks.setBorder(
				javax.swing.BorderFactory.createTitledBorder(resourceMap.getString("jTextAreaRemarks.border.title"))); // NOI18N
		...
		jListLinks.setBorder(
				javax.swing.BorderFactory.createTitledBorder(resourceMap.getString("jListLinks.border.title"))); // NOI18N
		...
		jPanel6Layout.setHorizontalGroup(jPanel6Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				...
				.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel6Layout.createSequentialGroup()
						.addContainerGap().addComponent(jTextFieldAddLink).addContainerGap()));
		jPanel6Layout.setVerticalGroup(jPanel6Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jPanel6Layout.createSequentialGroup()
						...
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addComponent(jTextFieldAddLink, javax.swing.GroupLayout.PREFERRED_SIZE,
								javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
						.addContainerGap()));
		...
		jPanel7Layout.setHorizontalGroup(jPanel7Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				...
				.addGroup(jPanel7Layout.createSequentialGroup().addContainerGap().addGroup(jPanel7Layout
						.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addComponent(jButtonAddAuthors)
						.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel7Layout.createSequentialGroup()
								.addComponent(jTextFieldFilterAuthorlist, javax.swing.GroupLayout.DEFAULT_SIZE, 266,
										Short.MAX_VALUE)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jButtonRefreshAuthorlist, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)))
						.addContainerGap()));
		jPanel7Layout.setVerticalGroup(jPanel7Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel7Layout.createSequentialGroup()
						...
						.addGroup(jPanel7Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
								.addComponent(jButtonRefreshAuthorlist, javax.swing.GroupLayout.DEFAULT_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
								.addComponent(jTextFieldFilterAuthorlist, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addComponent(jButtonAddAuthors).addContainerGap()));

		jTabbedPaneNewEntry1.addTab(resourceMap.getString("jPanel7.TabConstraints.tabTitle"), null, jPanel7,
				resourceMap.getString("jPanel7.TabConstraints.tabToolTip")); // NOI18N
		...
		jPanelQuickInputLayout.setHorizontalGroup(jPanelQuickInputLayout
				...
				.addGroup(jPanelQuickInputLayout.createSequentialGroup()
						.addGroup(jPanelQuickInputLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
								.addComponent(jCheckBoxQuickInput).addComponent(jComboBoxQuickInput,
										javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE,
										javax.swing.GroupLayout.PREFERRED_SIZE))
						.addContainerGap()));
		jPanelQuickInputLayout
				.setVerticalGroup(jPanelQuickInputLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jPanelQuickInputLayout.createSequentialGroup().addComponent(jCheckBoxQuickInput)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED,
										javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
								.addComponent(jComboBoxQuickInput, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)));
		...
		jPanel8Layout.setHorizontalGroup(jPanel8Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jPanel8Layout.createSequentialGroup().addContainerGap().addGroup(jPanel8Layout
						.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(jPanel8Layout.createSequentialGroup().addComponent(jButtonAddKeywords).addGap(0, 0, 0)
								.addComponent(jButtonQuickKeyword))
						.addComponent(jPanelQuickInput, javax.swing.GroupLayout.DEFAULT_SIZE,
								javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
						.addGroup(jPanel8Layout.createSequentialGroup().addComponent(jTextFieldFilterKeywordlist)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jButtonRefreshKeywordlist, javax.swing.GroupLayout.PREFERRED_SIZE, 16,
										javax.swing.GroupLayout.PREFERRED_SIZE)))
						.addContainerGap())
		...
		jPanel8Layout.setVerticalGroup(jPanel8Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel8Layout.createSequentialGroup()
						...
						.addGroup(jPanel8Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
								.addComponent(jButtonRefreshKeywordlist, javax.swing.GroupLayout.DEFAULT_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
								.addComponent(jTextFieldFilterKeywordlist, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addComponent(jPanelQuickInput, javax.swing.GroupLayout.PREFERRED_SIZE,
								javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addGroup(jPanel8Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
								.addComponent(jButtonAddKeywords).addComponent(jButtonQuickKeyword))
						.addContainerGap()));

		jTabbedPaneNewEntry1.addTab(resourceMap.getString("jPanel8.TabConstraints.tabTitle"), null, jPanel8,
				resourceMap.getString("jPanel8.TabConstraints.tabToolTip")); // NOI18N

		javax.swing.GroupLayout jPanel2Layout = new javax.swing.GroupLayout(jPanel2);
		jPanel2.setLayout(jPanel2Layout);
		jPanel2Layout.setHorizontalGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addComponent(jTabbedPaneNewEntry1, javax.swing.GroupLayout.DEFAULT_SIZE,
						javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE));
		jPanel2Layout.setVerticalGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addComponent(jTabbedPaneNewEntry1, javax.swing.GroupLayout.Alignment.TRAILING,
						javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE));
		...
		jPanel1Layout.setHorizontalGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel1Layout.createSequentialGroup()
						...
						.addComponent(jButtonCancel).addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addComponent(jButtonOK).addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
		...
		statusPanelLayout.setHorizontalGroup(statusPanelLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(statusPanelLayout.createSequentialGroup().addContainerGap().addComponent(jPanel1,
						javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
						.addContainerGap()));
		statusPanelLayout
				.setVerticalGroup(statusPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(statusPanelLayout.createSequentialGroup().addGap(0, 0, 0).addComponent(jPanel1,
		...
		layout.setHorizontalGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				...
				.addComponent(statusPanel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE,
						Short.MAX_VALUE)
				.addComponent(newEntryMainPanel, javax.swing.GroupLayout.DEFAULT_SIZE,
						javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE));
		layout.setVerticalGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(layout.createSequentialGroup()
						.addComponent(jToolBarNewEntry, javax.swing.GroupLayout.PREFERRED_SIZE,
								javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
						.addGap(0, 0, 0)
						.addComponent(newEntryMainPanel, javax.swing.GroupLayout.DEFAULT_SIZE,
								javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
