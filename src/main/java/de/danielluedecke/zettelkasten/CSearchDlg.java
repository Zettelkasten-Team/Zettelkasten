...
/**
 * Search dialog (Called Find in the UI).
 *
...
public class CSearchDlg extends javax.swing.JDialog {
	...
	/**
	 * Here we store the searchterms that are retrieved from the
	 * jTextFieldSearchterms. Since searchterms can be separated with commas, we
	 * might have several searchterms, thus using an array to store them...
	 */
	 ...
	 * might have several searchterms, thus using an array to store them...
	 * 
	 * @return a String-array that contains all search terms, or {@code null} if the
	...
	/**
	 * This method retrieves the logical of the combination of the search, i.e. if
	 * the {@link #searchterms} only match when all search terms are found within an
	 * entry (logical-and), or only one of them (logical-or) etc.
	 * 
	 ...
	 * Whether we have a wholeword search or not.
	 * 
	 * @return {@code true} when the search should match whole words only,
	 *         {@code false} when search terms also may match only parts within
	 *         words.
	 */
	...
	/**
	 * Whether synonyms should be included into the search or not. In case synonyms
	 * should be included, not only the direct search terms, but also possible
	 * relates synonyms (see {@code synonyms} class in the database-package).
	 * 
	...
	/**
	 * Whether search should be accent insensitive (treat a and á as the same)
	 */
	 ...
	 * Whether search should be accent insensitive (treat a and á as the same).
	 * 
	 * @return {@code true} when search should be accent insensitive, {@code false}
	 *         otherwise.
	 */
	...
	/**
	 * Indicates whether the search term is a <i>regular expression</i> or a
	 * "simple" search term.
	 * 
	...
	/**
	 * Indicates whether the user ticked the heckbox which starts a search for
	 ...
	 * period according to there creation or modfied date (timestamp-attriute of
	 * entries).
	 * 
	 * @return {@code true} when the user wants to limit the scope of the search to
	 *         entries that have been modified or created within a certain time
	 *         period, {@code false} if the search should include all entries,
	 *         independent from their timestamp-attribute.
	 */
	...
	/**
	 * Indicates whether the user, when starting a search within a certain time
	 * period (see {@link #isTimestampSearch() isTimestampSearch()}), wants to
	 * search for entries with a certain
	 * <ul>
	...
	private org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application
			...
			.getResourceMap(CSearchDlg.class);
	...
	public CSearchDlg(java.awt.Frame parent, SearchRequests sr, Settings s, String initialSearchTerm) {
		...
		jCheckBoxAccentInsensitive.setSelected((w & Constants.SEARCH_OPTION_ACCENT_INSENSITIVE) != 0);
		// init textfield
		...
		case 0:
			...
			break;
		case 1:
			jRadioButtonLogOr.setSelected(true);
			break;
		case 2:
			jRadioButtonLogNot.setSelected(true);
			break;
		default:
			jRadioButtonLogAnd.setSelected(true);
			break;
		...
		if (dates != null && dates.length > 1) {
			if (!dates[0].isEmpty())
				jFormattedTextFieldTimeFrom.setText(dates[0]);
			if (!dates[1].isEmpty())
				jFormattedTextFieldTimeTo.setText(dates[1]);
		}
		setMnemonicKeys();
	}
	...
	private void setMnemonicKeys() {
		if (!settingsObj.isMacStyle()) {
			...
			jCheckBoxAccentInsensitive.setDisplayedMnemonicIndex(0);
		}
	}
	...
	private void initListener() {
		...
		jComboBoxHistory.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				if (jComboBoxHistory.getSelectedIndex() > 0)
					jTextFieldSearchTerm.setText(jComboBoxHistory.getSelectedItem().toString());
			}
		});
		...
		jCheckBoxRegEx.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				jCheckBoxAccentInsensitive.setEnabled(!jCheckBoxRegEx.isSelected());
				jLabel2.setText(
						resourceMap.getString(jCheckBoxRegEx.isSelected() ? "jLabel2.textRegEx" : "jLabel2.text"));
	...
	public void startSearch() {
		...
		if (regex && !checkRegExPattern()) {
			...
			JOptionPane.showMessageDialog(null, resourceMap.getString("errWrongRegExMsg"),
					resourceMap.getString("errWrongRegExTitle"), JOptionPane.PLAIN_MESSAGE);
			// set focus back to textfield
			jTextFieldSearchTerm.requestFocusInWindow();
			// leave method
			return;
		}
		// get search terms and split them at each comma (comma seperates several
		// searchterms)
		...
		if (!regex) {
			...
			while (mat.find())
				result.add(mat.group(2) == null ? mat.group(3).trim() : mat.group(2).trim());
			// and copy the list to our array...
			searchterms = result.toArray(new String[result.size()]);
		}
		// else, if we have a regular expression, we do not split the search term after
		// each comma,
		...
		if (jRadioButtonLogAnd.isSelected())
			logical = Constants.LOG_AND;
		if (jRadioButtonLogOr.isSelected())
			logical = Constants.LOG_OR;
		if (jRadioButtonLogNot.isSelected())
			logical = Constants.LOG_NOT;

		wholeword = jCheckBoxWholeWord.isSelected();
		matchcase = jCheckBoxMatchCase.isSelected();
		synonyms = jCheckBoxSynonyms.isSelected();
		accentInsensitive = jCheckBoxAccentInsensitive.isSelected();

		where = 0;
		// now check where the user wants to search in...
		if (jCheckBoxSTitles.isSelected())
			where = where | Constants.SEARCH_TITLE;
		if (jCheckBoxSContent.isSelected())
			where = where | Constants.SEARCH_CONTENT;
		if (jCheckBoxSAuthors.isSelected())
			where = where | Constants.SEARCH_AUTHOR;
		if (jCheckBoxSKeywords.isSelected())
			where = where | Constants.SEARCH_KEYWORDS;
		if (jCheckBoxSRemarks.isSelected())
			where = where | Constants.SEARCH_REMARKS;
		if (jCheckBoxSLinks.isSelected())
			where = where | Constants.SEARCH_LINKS;
		if (jCheckBoxSLinksContent.isSelected())
			where = where | Constants.SEARCH_LINKCONTENT;
		...
		int options = 0;
		if (wholeword)
			options = options | Constants.SEARCH_OPTION_WHOLEWORD;
		if (matchcase)
			options = options | Constants.SEARCH_OPTION_MATCHCASE;
		if (synonyms)
			options = options | Constants.SEARCH_OPTION_SYNONYMS;

		settingsObj.setSearchOptions(options);

		// save logical combination
		if (jRadioButtonLogAnd.isSelected())
			settingsObj.setSearchLog(0);
		else if (jRadioButtonLogOr.isSelected())
			settingsObj.setSearchLog(1);
		else if (jRadioButtonLogNot.isSelected())
			settingsObj.setSearchLog(2);
		...
		if (!date1.isEmpty() && !date2.isEmpty())
			settingsObj.setSearchDateTime(date1 + "," + date2);
		// check whether the date-period was entered correctly, so we don't have for
		// instance a later
		...
		if (istimesearch) {
			try {
				...
				// display error message box
				JOptionPane.showMessageDialog(null, resourceMap.getString("errWrongDateMsg"),
						resourceMap.getString("errWrongDateTitle"), JOptionPane.PLAIN_MESSAGE);
				// set focus back to textfield
	...
	private void initComponents() {
		...
		jCheckBoxAccentInsensitive = new javax.swing.JCheckBox();
		jButtonCancel = new javax.swing.JButton();
		jButtonSearch = new javax.swing.JButton();
		jComboBoxHistory = new javax.swing.JComboBox<String>();
		...
		jComboBoxTimeSearch = new javax.swing.JComboBox<String>();
				...
				.addGroup(jPanel2Layout.createSequentialGroup().addContainerGap().addGroup(jPanel2Layout
						...
						.addGroup(jPanel2Layout.createSequentialGroup()
								.addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
										.addComponent(jCheckBoxSTitles).addComponent(jCheckBoxSContent)
										.addComponent(jCheckBoxSAuthors))
								.addGap(18, 18, 18)
								.addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
										.addComponent(jCheckBoxSRemarks).addComponent(jCheckBoxSLinks)
		...
		jPanel2Layout.setVerticalGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jPanel2Layout.createSequentialGroup().addContainerGap()
						.addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
								.addGroup(jPanel2Layout.createSequentialGroup().addComponent(jCheckBoxSRemarks)
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
										.addComponent(jCheckBoxSLinks)
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
										.addComponent(jCheckBoxSLinksContent))
								.addGroup(jPanel2Layout.createSequentialGroup().addComponent(jCheckBoxSTitles)
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
										.addComponent(jCheckBoxSContent)
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
										.addComponent(jCheckBoxSAuthors)))
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addComponent(jCheckBoxSKeywords).addContainerGap()));
		...
		jCheckBoxAccentInsensitive.setText(resourceMap.getString("jCheckBoxAccentInsensitive.text")); // NOI18N
		jCheckBoxAccentInsensitive.setName("jCheckBoxAccentInsensitive"); // NOI18N
		...
		jPanel5Layout.setHorizontalGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jPanel5Layout.createSequentialGroup().addContainerGap()
						.addGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
								.addComponent(jRadioButtonLogNot).addComponent(jRadioButtonLogOr)
								.addComponent(jRadioButtonLogAnd))
						.addGap(18, 18, 18)
						.addGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
								.addComponent(jCheckBoxWholeWord).addComponent(jCheckBoxMatchCase)
								.addComponent(jCheckBoxSynonyms).addComponent(jCheckBoxAccentInsensitive))
						.addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)));
		jPanel5Layout.setVerticalGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jPanel5Layout.createSequentialGroup().addContainerGap()
						.addGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
								.addGroup(jPanel5Layout.createSequentialGroup().addComponent(jCheckBoxWholeWord)
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
										.addComponent(jCheckBoxMatchCase)
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
										.addComponent(jCheckBoxSynonyms)
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
										.addComponent(jCheckBoxAccentInsensitive))
								.addGroup(jPanel5Layout.createSequentialGroup().addComponent(jRadioButtonLogAnd)
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
		...
		jFormattedTextFieldTimeFrom.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(
		...
		jFormattedTextFieldTimeTo.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(
				new javax.swing.text.DateFormatter(java.text.DateFormat.getDateInstance(java.text.DateFormat.SHORT))));
		jFormattedTextFieldTimeTo.setName("jFormattedTextFieldTimeTo"); // NOI18N

		jComboBoxTimeSearch.setModel(
				new javax.swing.DefaultComboBoxModel<String>(new String[] { "Erstelldatum", "Änderungsdatum", "Beides" }));
		...
		jPanel1Layout.setHorizontalGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jPanel1Layout.createSequentialGroup().addContainerGap().addGroup(jPanel1Layout
						...
						.addGroup(jPanel1Layout.createSequentialGroup().addComponent(jCheckBoxTimeSearch)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jFormattedTextFieldTimeFrom, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jLabelTimeSearch)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(jFormattedTextFieldTimeTo, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED).addComponent(
										jComboBoxTimeSearch, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
						.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel1Layout.createSequentialGroup()
								.addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
										...
										.addGroup(javax.swing.GroupLayout.Alignment.LEADING, jPanel1Layout
												...
												.addGroup(jPanel1Layout
														.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
														.addComponent(jComboBoxHistory, 0,
																javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
						...
						.addGroup(javax.swing.GroupLayout.Alignment.TRAILING,
								jPanel1Layout.createSequentialGroup().addComponent(jButtonCancel)
										.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
		...
		jPanel1Layout.setVerticalGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(jPanel1Layout.createSequentialGroup().addContainerGap().addComponent(jLabel2)
						...
						.addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
								.addComponent(jLabel1).addComponent(jTextFieldSearchTerm,
										javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE,
										javax.swing.GroupLayout.PREFERRED_SIZE))
						.addGap(0, 0, 0).addComponent(jCheckBoxRegEx)
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						.addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
								.addComponent(jLabel3).addComponent(jComboBoxHistory,
										javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE,
										javax.swing.GroupLayout.PREFERRED_SIZE))
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
						...
						.addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
								...
								.addComponent(jFormattedTextFieldTimeTo, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
								.addComponent(jComboBoxTimeSearch, javax.swing.GroupLayout.PREFERRED_SIZE,
										javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
						.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED, 11, Short.MAX_VALUE)
						.addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
								.addComponent(jButtonCancel).addComponent(jButtonSearch))));
	...
	private javax.swing.JCheckBox jCheckBoxAccentInsensitive;
	private javax.swing.JCheckBox jCheckBoxTimeSearch;
	private javax.swing.JCheckBox jCheckBoxWholeWord;
	private javax.swing.JComboBox<String> jComboBoxHistory;
	private javax.swing.JComboBox<String> jComboBoxTimeSearch;
