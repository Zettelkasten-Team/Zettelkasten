...
/**
 * Accelerator keys are the keyboard shortcuts of the application. This class
 * keeps tracks of the name, keys, and action of each keyboard shortcut.
 * 
...
public class AcceleratorKeys {
	...
	/**
	 * The accelerator keys class. This class manages the accelerator keys. The user
	 * can define own accelerator keys for each relevant action. Retrieving and
	 * setting this user defined data is done by this class. <br>
	 * <br>
	 * An XML-File could look like this:<br>
	 * <br>
	 * &lt;acceleratorkeys&gt;<br>
	 * &nbsp;&nbsp;&lt;key action=&quot;newEntry&quot;&gt;control n&lt;/key&gt;<br>
	 * &nbsp;&nbsp;&lt;key action=&quot;openDocument&quot;&gt;control
	 * o&lt;/key&gt;<br>
	 * &lt;/acceleratorkeys&gt;<br>
	 */
	public AcceleratorKeys() {
		...
		initDefaultAcceleratorKeysIfMissing();
	}

	/**
	 * initDefaultAcceleratorKeysIfMissing creates all the used accelerator keys in
	 * the XML documents, setting them to the default value if missing.
	 * 
	...
	public final void initDefaultAcceleratorKeysIfMissing() {
		...
		initDefaultNewEntryKeysIfMissing();
		initDefaultDesktopKeysIfMissing();
		initDefaultSearchResultsKeysIfMissing();
	}

	/**
	 * This method inits the accelerator table of the main window's menus. We
	 * separate the initialisation of the accelerator tables for each window to keep
	 * an better overiew.
	 * 
	 * This method creates all the acceleratorkeys-child-elements, but only, if they
	 * don't already exist. We do this because when loading older
	 * acceleratorkeys-xml-document-structures, we might have new elements that
	 * would not be initialised. but now we can call this method after loading the
	...
	void initDefaultMainKeysIfMissing() {
		...
		// now we have to go through an endless list of accelerator keys. it is
		// important
		// that the attribute values have exactly the same spelling like the actions'
		// names
		// which can be found in the properties-files (resources). This ensures we can
		// easily
		...
		// this);
		// AbstractAction ac = (AbstractAction)
		// actionMap.get(CAcceleratorKey.getActionName());
	...
	/**
	 * This methos inits the accelerator table of the new entry window's menus. We
	 * separate the initialisation of the accelerator tables for each window to keep
	 * an better overiew.
	 */
	...
	/**
	 * This methos inits the accelerator table of the desktop window's menus. We
	 * separate the initialisation of the accelerator tables for each window to keep
	 * an better overiew.
	 */
	...
	/**
	 * Retrieves an xml-file with the requested accelerator information. This method
	 * is called from within the CSettings-class, which handles the loading and
	 * saving of these xml files.
	 * 
	...
	public Document getDocument(int what) {
		switch (what) {
		case MAINKEYS:
			return acceleratorKeysMain;
		case NEWENTRYKEYS:
			return acceleratorKeysNewEntry;
		case DESKTOPKEYS:
			return acceleratorKeysDesktop;
		case SEARCHRESULTSKEYS:
			return acceleratorKeysSearchResults;
		default:
			throw new RuntimeException(String
					.format("FOUND BUG! AcceleratorKeys.getDocument() received invalid document type [%d]", what));
		}
	}

	/**
	 * This method sets an accelerator file. This method called from within the
	 * CSettings class where the data is loaded and the file/information is passed
	 * to this method.
	 * 
	 * Following constants should be used as parameters:<br>
	 * MAINKEYS<br>
	 ...
	 * @param af
	 */
	public void setDocument(int what, Document newDocument) {
		switch (what) {
		case MAINKEYS:
			acceleratorKeysMain = newDocument;
			break;
		case NEWENTRYKEYS:
			acceleratorKeysNewEntry = newDocument;
			break;
		case DESKTOPKEYS:
			acceleratorKeysDesktop = newDocument;
			break;
		case SEARCHRESULTSKEYS:
			acceleratorKeysSearchResults = newDocument;
			break;
		default:
			throw new RuntimeException(String
					.format("FOUND BUG! AcceleratorKeys.setDocument() received invalid document type [%d]", what));
		}
	}
	 ...
	 * @return the string containing the accelerator key or an empty string if
	 *         nothing was found
	 */
	 ...
	 * @return the string containing the accelerator key or null if nothing was
	 *         found
	 */
	...
	/**
	 * This method sets an accelerator key of an related action. To change an
	 * accelerator key, provide the action's name and the keystroke-value as string
	 * parameters. furthermore, we have to tell the method, to which file the
	 * changes should be applied (param what).
	 * 
	 * Following constants should be used as parameters:<br>
	 * MAINKEYS<br>
	 * NEWENTRYKEYS<br>
	 ...
	 * @return {@code true} if we have an element that contains that attribute,
	 *         false otherwise
	 */
	 ...
	 * @return the element which matches the given action-name {@code attr} inside
	 *         the document {@code doc}, or null if no match was found
	 */
	 ...
	 * position is a value from 1 to (size of xml file) - in contrary to usual array
