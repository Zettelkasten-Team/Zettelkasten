...
public class KeywordSuggestionsTask extends org.jdesktop.application.Task<Object, Void> {
    ...
    /**
     * The final results of the quick input. Contains the keywords that have to be set to the keyword
     * list in the new entry-frame, when this task is finished.
     */
        ...
        switch (quickstep) {
            ...
            case 1:
                ...
                for (int cnt=1; cnt<=dataObj.getCount(Daten.KWCOUNT); cnt++) {
                    ...
                    if (!kw.isEmpty()) remainingKeywords.add(kw);
                }
                ...
                while (i.hasNext()) {
                    ...
                    if (settingsObj.getSearchAlwaysSynonyms()) synline = synonymsObj.getSynonymLine(kw, false);
                    // if we don't have any synonyms, put only keyword in the array
                    if (null==synline) synline = new String[] {kw};
                    ...
                    for (String s : synline) {
                        ...
                        if (entrytext.contains(s.toLowerCase())) {
                            newKeywords.add(kw);
                ...
                if (Settings.QUICK_INPUT_MORE==extendedQuickInput) {
                    ...
                    while (i.hasNext()) {
                        ...
                        for (String s : synline) {
                            ...
                            if ((s.length()>3)&&(entrytext.contains(s.toLowerCase()))) {
                                found = true;
                                // leave loop
                                break;
                            }
                ...
                if (newKeywords!=null && !newKeywords.isEmpty()) Collections.sort(newKeywords, new Comparer());
            ...
            case 3:
                ...
                if ((fromFirstStep!=null)&&(fromFirstStep.size()>0)) {
                    ...
                    if (newKeywords!=null && !newKeywords.isEmpty()) Collections.sort(newKeywords, new Comparer());
                    if (remainingKeywords!=null && !remainingKeywords.isEmpty()) Collections.sort(remainingKeywords, new Comparer());
                }
                // we're done with the first step
                break;
        }
