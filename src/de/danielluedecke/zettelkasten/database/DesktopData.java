...
/**
 * The root-element is names "desktops". For each desktop, a new child named "desktop" is created. This
 * child holds the content for one desktop.
 * 
 * The first Element is always "bullet", a bullet-point. Bullet-points do always have the attribute "name"
 * which holds the name of the bullet-point that is displayed in the jTree. Furthermore, each bullet-element
 * automatically creates a child-element "comment", even if a bullet-point has no comments.
 * 
 * "comment" is therefor always the first child-element of "bullet".
 * 
 * Futher child-elements are the entries, named "entry". Each entry-element has an id with the related
 * entry-number from the main-datafile (see CDaten-class) and a timestamp which indicates when this
 * entry was added to the desktop.
 * 
...
public class DesktopData {
    ...
    /**
     * Constants for the method {@link #importArchivedDesktop(org.jdom.Document) importArchivedDesktop()}.
     * Indicates that the import of an desktop-archive was successful.
     */
    public static final int IMPORT_ARCHIVE_OK = 1;
    /**
     * Constants for the method {@link #importArchivedDesktop(org.jdom.Document) importArchivedDesktop()}.
     * Indicates that the import of an desktop-archive failed, because the desktop-name of the imported
     * archive already exists.
     */
    public static final int IMPORT_ARCHIVE_ERR_DESKTOPNAME_EXISTS = 2;
    /**
     * Constants for the method {@link #importArchivedDesktop(org.jdom.Document) importArchivedDesktop()}.
     * Indicates that the import of an desktop-archive failed, because a technical error occured.
     */
    public static final int IMPORT_ARCHIVE_ERR_OTHER = 3;
    /**
     * This element stores a bullet-element that is currently in the clipboard - either due to
     * a cut or copy operation from the CDesktop-Dialog
     */
    ...
    /**
     * This method returns the index-number for the currently used desktop-data. This is
     * necessary since a desktop-document may contain several desktop-data-units.
     * 
     * @return the index-number of the currently used desktop, starting with {@code 0} for
     * the first desktop-data
     */
    public int getCurrentDesktopNr() {
        return currentDesktop;
    }
    /**
     * This method sets the index-number for the currently used desktop-data. This is
     * necessary since a desktop-document may contain several desktop-data-units.
     * 
     * @param nr the number of the current desktop, using {@code 0} as the first index number.
     */
     ...
     * @return the amount of desktop-data-units. returns {@code 0} if no desktop data exists.
     */
    public int getCount() {
     ...
     *
     * @return a String containing the name of the currently active desktop or a string {@code noDesktopNameFound}
    ...
    public String getDesktopName(int nr) {
        ...
        if (null==d) return resourceMap.getString("noDesktopNameFound");
        // get the desktop's name-attribute
        String retval = d.getAttributeValue("name");
        // check for content
        if ((null==retval)||(retval.isEmpty())) retval = resourceMap.getString("noDesktopNameFound");
        // and return it
        return retval;
    }
    /**
    ...
    /**
     * This method recursivly searches through all entris of the desktop {@code desktopnr} and
     * looks for occurences of the entry with the numer {@code entrynr}. If an entry was found,
     * true is returned, otherwise false is returned.<br><br>
     * This method can be used to look for multiple entry occurences not only in the current sub-tree
     * or sub-bullet, but within all desktop-files - so the user knows that he has already added the
     * entry {@code entrynr} to this or another existing desktop in the past.
     *
    ...
    public boolean checkForDoubleEntry(int desktopnr, int entrynr) {
        ...
        if (null==desktopelement) return false;
        // find elements
     ...
     * from all available desktop-elements.
     * 
    ...
    public boolean deleteEntries(int[] entries) {
                ...
                for (int e : entries) {
                    ...
                    if (lle!=null && lle.size()>0) {
                        ...
                        while (prepare.hasNext()) {
                            ...
                            if (entry!=null) finallist.add(entry);
                        }
                    }
                }
            }
        ...
        // change modified state
        if (haveDeletedEntries) setModified(true);
        // return result
    ...
    /**
     * This method recursivles searches through all entris of the desktop {@code desktopnr} and
     * looks for occurences of the entry with the numer {@code entrynr}. If an entry was found,
     * this entry is returned as {@code Element}, otherwise {@code null} is returned.<br><br>
     * This method can be used to look for entries that should be removed from the desktop.
     *
     * @param desktopnr the desktop where we should look for occurences of the entry {@code entrynr}
     * @param entrynr the number of the entry that should be looked for. usually, this number corresponds
     * to entry-numbers of entries that are currently being added
     * @return a linked list of type {@code Element}, which contains the found entry-Elements, in the desktop
     * {@code desktopnr}. If no Elements (entries) have been found, the method return {@code null}.
     */
    public LinkedList<Element> searchForEntry(int desktopnr, int entrynr) {
        // retrieve desktop-element
        Element desktopelement = getDesktopElement(desktopnr);
        // if desktop-element does not exist, return false
        if (null==desktopelement) return null;
        // find elements
        LinkedList<Element> retval = findElements(desktopelement,String.valueOf(entrynr),new LinkedList<Element>());
        // if we found something, return linked list, else return null
    ...
    private LinkedList<Element> findElements(Element e, String entrynumber, LinkedList<Element> foundelements) {
        ...
        if (e==null) return foundelements;
        ...
        while (it.hasNext()) {
            ...
            if (hasChildren(e)) foundelements = findElements(e,entrynumber,foundelements);
            ...
            if (e!=null && e.getName().equals(ELEMENT_ENTRY)) {
                ...
                if (att!=null && att.equals(entrynumber)) foundelements.add(e);
            }
        }
    ...
    public int getCommentCount(Element e, int commentcount) {
        ...
        while (it.hasNext()) {
            ...
            e = it.next();
            // else check whether we have child-elements - if so, re-call method
            if (hasChildren(e)) commentcount = getCommentCount(e,commentcount);
            // check whether we have an entry-element that matched the requested id-number
            if (e!=null) {
                ...
                if (e.getName().equals(ELEMENT_BULLET)) {
                    ...
                    if (comel!=null && !comel.getText().isEmpty()) commentcount++;
                }
                else {
                    // else return the element's text
                    if (!e.getText().isEmpty()) commentcount++;
                }
            }
        }
        return commentcount;
    ...
    public boolean desktopHasElement(Element e, String entrynumber, boolean found) {
        ...
        while (it.hasNext()) {
            ...
            e = it.next();
            // else check whether we have child-elements - if so, re-call method
            if (hasChildren(e)) found = desktopHasElement(e,entrynumber,found);
    ...
    /**
     * This method checks whether an entry with the number {@code entrynumber} is stored in any
     * of the available desktops. If any of the desktops contains an entry with the number {@code entrynumber},
     * {@code true} is returned.
     * 
     * @param entrynumber the number of that entry that should be found in the desktops.
     * @return {@code true} if this entry was found in any desktop, {@code false} otherwise or if no desktops exist.
     */
    ...
    /**
     * This method checks whether an entry with the number {@code entrynumber} is stored in the current desktop.
     * If this desktop has an entry with the number {@code entrynumber},
     * {@code true} is returned.
     * 
     * @param entrynumber the number of that entry that should be found in the current desktop.
     * @return {@code true} if this entry was found in this desktop, {@code false} otherwise or if no such desktop exist.
     */
    public boolean isEntryInCurrentDesktop(int entrynumber) {
        return isEntryInDesktop(entrynumber, getCurrentDesktopNr());
    }
    /**
     * This method checks whether an entry with the number {@code entrynumber} is stored in the desktops with
     * the desktop-nr {@code desktopnr}. If this desktop has an entry with the number {@code entrynumber},
     * {@code true} is returned.
     * 
     * @param entrynumber the number of that entry that should be found in the desktop with the index-number {@code desktopnr}.
     * @param desktopnr the number of the desktop where the entry should be searched for.
     * @return {@code true} if this entry was found in this desktop, {@code false} otherwise or if no such desktop exist.
     */
    public boolean isEntryInDesktop(int entrynumber, int desktopnr) {
        ...
        if (desktopnr<0 || desktopnr>getCount()) return false;
        // check whether the desktop has this entry as element
    ...
    /**
     * If we have an element found with {@link #desktopHasElement(org.jdom2.Element, java.lang.String, boolean)}
     * we can get this {@code Element} with this function for further use.
     * @return the Element of an entry that was found on a desktop (via {@link #desktopHasElement()}, or
     * {@code null} if no element / entry was found.
     */
     ...
     * @param entrynr the number of the entry that should be searched for
     * in all desktops
     ...
     * @return the element of the requested desktop-data (dektop including child-elements like
     * bullets and nodes), or null if an error occured
     */
    ...
    /**
     * This method returns a desktop-element-unit of the desktop with the name {@code desktopname}.
     * If no such desktop exists that matches the parameter {@code desktopname}, {@code null}
     * is returned.
     *
     * @param desktopname the name of the desktop of which element we want to retrieve
     * @return the element of the requested desktop-data (dektop including child-elements like
     * bullets and nodes), or |@code null} if an error occured
     */
     ...
     * 
     * @return the currently used desktop as jdom-element, or null if an error occured
     */
    ...
    public boolean addNewDesktop(String name, String[] notes) {
        ...
        for (int cnt=0; cnt<getCount(); cnt++) if (name.equalsIgnoreCase(getDesktopName(cnt))) return false;
        // create new element
    ...
    public String addBullet(String timestamp, String name) {
        ...
        if (parentb!=null) {
            ...
            try {
                ...
                while (timeStampExists(newts)) newts = Tools.getTimeStampWithMilliseconds();
     ...
     * a global Element-variable, so this element is stored for later use.
     *
     * @param timestamp the timestamp of the bullet that should be copied to the "clipboard"
     */
    ...
    public String addEntry(String timestamp, String nr, int insertpos) {
        ...
        if (b!=null) {
            ...
            if (!b.getName().equals(ELEMENT_BULLET)) b = b.getParentElement();
            // create a new element
            ...
            try {
                ...
                // check whether timestamp already exists. this is particulary the
                // case when a user adds several entries at once.
                while (timeStampExists(ts)) ts = Tools.getTimeStampWithMilliseconds();
    ...
    /**
     * This method removes the element with the timestamp-attribute {@code timestamp}
     * from the list of modified entries.
     *
     * @param timestamp the timestamp of that entry that should be removed from the modification-list
     */
    ...
    /**
     * This methods checks whether a given desktop "nr" has any child-elements (i.e. bullets)
     * or not.
     * 
    ...
    /**
     * Gets the comment of the selected entry in the jTree in the CDesktop-frame. This method
     * traverses the xml-datafile, looking for each element and checks whether the elements
    ...
    /**
     * This method retrieves all entry-numbers of the current acticated desktop in ascending sorted
     * order and returns them as an integer-array. multiple entry-numbers are removed, so this array
     * contains each entry-number only once.
     *
     * @return an integer-array with all entry-numbers of the current activated desktop in ascending sorted
     * order. multiple entry-numbers are removed, so this array
     * contains each entry-number only once.
     */
    ...
    /**
     * This method retrieves all entry-numbers of the desktop {@code deskopnr} in ascending sorted
     * order and returns them as an integer-array. multiple entry-numbers are removed, so this array
     * contains each entry-number only once.
     *
     * @param desktopnr the desktop-number of the desktop which entries should be retrieved.
     * @return an integer-array with all entry-numbers of the desktop {@code deskopnr} in ascending sorted
     * order. multiple entry-numbers are removed, so this array
     * contains each entry-number only once.
     */
    ...
    /**
     * This method retrieves all entry-numbers of element {@code bullet} in ascending sorted
     * order and returns them as an integer-array. multiple entry-numbers are removed, so this array
     * contains each entry-number only once.
     *
     * @param bullet the (bullet-)element of the desktop which entries should be retrieved.
     * @return an integer-array with all entry-numbers of {@code bullet} in ascending sorted
     * order. multiple entry-numbers are removed, so this array
     * contains each entry-number only once.
     */
    ...
    /**
     * This method retrieves all entry-timestamps of the desktop {@code desktopnr} and returns them as an string-array.
     *
     * @param desktopnr the number of of the desktop which entry-timestamps should be retrieved.
     * @return a string-array with all entry-timestamps of those entry with the parent
     * {@code bullet}.
     */
    ...
    /**
     * This method retrieves all entry-timestamps of those entry which are children of the
     * element {@code bullet} and returns them as an string-array.
     *
     * @param bullet the (bullet-)element of the desktop which entries should be retrieved.
     * @return a string-array with all entry-timestamps of those entry with the parent
     * {@code bullet}.
        ...
        while (it.hasNext()) {
            ...
            if (!e.getName().equals(ATTR_COMMENT)) {
                ...
                if(!e.getName().equals(ELEMENT_BULLET)) {
                    ...
                    if (att!=null) timestampList.add(att);
                }
                // when the new element also has children, call this method again,
                // so we go through the strucuture recursively...
                if (hasChildren(e)) {
    ...
    /**
     * This method retrieves the content of a modified entry. the modified entries' content is stored
     * in a separated XML-Document (see {@link #modifiedEntries modifiedEntries}. each element of this
     * document has a timestamp-attribute that equals the timestamp-attribute of an entry in the
     * {@link #desktop desktop}-Document.
     * <br><br>
     * So, by passing a {@code timestamp} value, this method searches whether we have any modified entry
     * that has the same timestamp-attribut, and if so, it returns the content of that element, which was
     * modified (and thus differs from an entry's content as it is stored in the original database).
     *
     * @param timestamp the timestamp which should match the requested entry's timestamp-attribute
     * @return the content of the modified entry, or {@code null} if no entry was found.
     */
    public String retrieveModifiedEntryContentFromTimestamp(String timestamp) {
        ...
        // check for content
        if (retval!=null) return retval.getText();
        // else return null
        return null;
    }
    /**
     ...
     * <br><br>
     * By passing a {@code timestamp} value, this method searches whether we have any entry
     * that has the same timestamp-attribut, and if so, it returns the original content of that element.
     *
     * @param timestamp the timestamp which should match the requested entry's timestamp-attribute
    ...
    /**
     * This method retrieves the element of a modified entry. the modified entries' content is stored
     * in a separated XML-Document (see {@link #modifiedEntries modifiedEntries}. each element of this
     * document has a timestamp-attribute that equals the timestamp-attribute of an entry in the
     * {@link #desktop desktop}-Document.
     * <br><br>
     * So, by passing a {@code timestamp} value, this method searches whether we have any modified entry
     * that has the same timestamp-attribut, and if so, it returns that element which was
     * modified (and thus differs from an entry's content as it is stored in the original database).
     *
     * @param timestamp the timestamp which should match the requested entry's timestamp-attribute
    ...
    /**
     * This method checks whether an entry with the timestamp {@code timestamp} has been modified
     * or not. that means, the methods looks for any elements in the {@link #modifiedEntries modifiedEntries}
     * document that match the timestamp-attribute.
     *
     * @param timestamp the timestamp of the possible modified entry.
     * @return {@code true} if the entry with the timestamp-attribute {@code timestamp} was modified before,
    ...
    public boolean addModifiedEntry(String timestamp, String content) {
        ...
        if (null==timestamp || timestamp.isEmpty()) return false;
        ...
        while (it.hasNext()) {
            ...
            try {
                if (e.getName().equals(ELEMENT_ENTRY)) {
                    ...
                    if (timestamp!=null) e.setAttribute(ATTR_TIMESTAMP, timestamp.concat(df.format(timestampid++)));
                }
                if (e.getName().equals(ELEMENT_BULLET)) {
    ...
    public Element findEntryElementFromTimestamp(Element e, String t) {
        ...
        if (e.getAttributeValue(ATTR_TIMESTAMP)!=null && e.getAttributeValue(ATTR_TIMESTAMP).equals(t)) return e;
        // get a list with all children of the element
        List<Element> children = e.getChildren();
        // create an iterator
        Iterator<Element> it = children.iterator();
        ...
        while (it.hasNext()) {
            ...
            if (hasChildren(e)) {
                ...
                if (e!=null && e.getAttributeValue(ATTR_TIMESTAMP)!=null && e.getAttributeValue(ATTR_TIMESTAMP).equals(t)) return e;
            }
        }
        if (e!=null) return (e.getAttributeValue(ATTR_TIMESTAMP)!=null && e.getAttributeValue(ATTR_TIMESTAMP).equals(t)) ? e : null; else return null;
    }


    /**
     * This method retrieves the level (depth) of an bullet-element within the tree-hierarchy. This
     * method can be used for example to determine the level of a heading, when exporting desktop-data.
     * 
     * @param t the timestamp which should match the timestamp-attribute of the entry-element
     * @return the level (depth) of the bullet within the tree-hierarchy as integer-value,
     ...
     * @return the archived document as XML-document, or {@code null} if an error occured.
     */
    ...
    public int importArchivedDesktop(Document archive) {
        ...
        if (null==name || name.isEmpty()) return IMPORT_ARCHIVE_ERR_DESKTOPNAME_EXISTS;
        // first of all, go through all desktops and check whether the name
        // already exist, to avoid double naming...
        // when such a desktopname as "name" already exists, return false
        for (int cnt=0; cnt<getCount(); cnt++) if (name.equalsIgnoreCase(getDesktopName(cnt))) return IMPORT_ARCHIVE_ERR_DESKTOPNAME_EXISTS;
        // create new element
        Element d = new Element("desktop");
        try {
            // set the desktop's name as attribute
            ...
            if (noteschild!=null) {
                ...
                if (nc1!=null) n1.setText(nc1.getText());
                // get and add notes...
                Element nc2 = noteschild.getChild("notes2");
                if (nc2!=null) n2.setText(nc2.getText());
                // get and add notes...
                Element nc3 = noteschild.getChild("notes3");
                if (nc3!=null) n3.setText(nc3.getText());
            }
            // add notes-sub-elements
            desk.addContent(n1);
            desk.addContent(n2);
