...
public class FileOperationsUtil {
    ...
    /**
     * This method creates a backup-filepath by adding the extension <b>.backup</b> to
     * the filepath given in {@code sourcefile}. In case this filepath already exists,
     * this method tries to use a new extension, until a non-existing filepath was
     * created.<br><br>
     * Results might be:<br>
    ...
    /**
     * Creates a mac-aqua-like file dialog. Since the typical OS X dialog can not be created using the
     * swing file chooser, we use the awt-FileDialog instead to get mac-like-look'n'feel.
     *
     ...
     * @return a reference to the created <i>and initiated</i> FileDialog which was passed as parameter {@code fc}.
     */
    ...
    /**
     * This method returns the file extension of a given file passed as string-parameter.
     * 
     * @param settingsObj
     * @param dataObj
     * @param f the file which extension should be retrieved
    ...
    /**
     * This method copies a file. check for existing files is not done here, needs to be
     * checked from within the method which calls this method.
     *
     ...
     * @return the selected directory as File-variable, or {@code null} if user cancels the dialog
     */
            ...
            for (File f: sources) {
                ...
                while (!dest_ok) {
                    ...
                    if (dest.exists()) {
                        ...
                        if (ATT_CANCEL_RENAME==msgOpt) return false;
    ...
    /**
     * This methods returns the path to an entry's attachment-file, which is passed as paraneter
     * {@code linktye}.
     * It creates an absolute path out from a relative path if necessray, which e.g. occures
     * when having entry-attachments.
     *
     * @param settingsObj a reference to the CSettings-class
     * @param dataObj
     * @param linktype a hyperlink-file, which is received when the user clicks on a hyperlink
     * or linked attachment in e jEditorPane
    ...
    /**
     * This method checks whether the file's {@code imgfile} extenstion
     * is corresponding to an image file, i.e. whether the file extension
     * equals one of the most common image formats.
     *
    ...
    /**
     * This method retrieves a complete formtag, extracts the information and converts
     * the tag into the related image-path, which can be used for including the related
     * form-image (which is stored as image-file in the subdirectory {@code Constants.FORMIMAGEPATH_SUBDIR}.
     *
     * @param formtag the form-tag, which contains the form-data in UBB-format
     * @param addFileExtension
     * @param addLargeAppendix
     * @return a string containing the file-name of the related form-image that represents the
     * graphical darstellung of the form-tag, or an empty string if an error occured.
     */
    ...
    /**
     * This method checks whether the file's {@code imgfile} extenstion
     * is corresponding to an image file and whether this image format
     * is supported by the JEditorPane component.
     *
     * @param imgfile the file which extension should be checked
     * @return {@code true} if the the file seems to be an image which is also supported to be
     * displayed in a JEditorPane, {@code false} otherwise.
     */
    ...
    /**
     * This method checks whether a given string, usually passed as parameter from the
     * {@link #eventHyperlinkActivated(javax.swing.event.HyperlinkEvent) eventHyperlinkActivated},
     * is a hyperlink or not. this is decided from the prefix, i.e. whether the string starts with
     ...
     * @return The {@code .Zettelkasten} directory, which is a subdirectory of the user.home
     * directory, including a trailing separator char ({@code <user-home>/.Zettelkasten/}).
     */
    ...
    /**
     * Retrieve the {@code .Zettelkasten} subdirectory of the user home directory.
     * @param addTrailingSeparatorChar if {@code true}, a trailing separator char is added.
     * @return The {@code .Zettelkasten} directory, which is a subdirectory of the user.home
     * directory.
     */
    public static String getZettelkastenHomeDir(boolean addTrailingSeparatorChar) {
        ...
        if (addTrailingSeparatorChar) fp = fp+File.separatorChar;
        return fp;
    }
}
